...
test('Wam webhook HMAC verification accepts a valid signature and rejects a bad one', async () => {
  const crypto = require('node:crypto');
  // load wam with a test secret via env
  const prev = process.env.WAM_WEBHOOK_SECRET;
  process.env.WAM_WEBHOOK_SECRET = 'test-secret';
  delete require.cache[require.resolve('../src/services/wam')];
  const wam = require('../src/services/wam');
  const body = JSON.stringify({ event_type: 'payment.completed', paymentId: 'pay_123' });
  const ts = '1700000000';
  const sig = crypto.createHmac('sha256', 'test-secret').update(ts + '.' + body).digest('hex');
  assert.equal(wam.verifyWebhook({ body, signature: sig, timestamp: ts }), true);
  assert.equal(wam.verifyWebhook({ body, signature: 'deadbeef', timestamp: ts }), false);
  assert.equal(wam.eventStatus(JSON.parse(body)), 'PAID');
  process.env.WAM_WEBHOOK_SECRET = prev;
});

test('payment can be marked paid via Wam payment reference', () => {
  store.reset();
  const buyer = store.createUser({ email: 'wb@x.com', name: 'W', org_name: 'WC' });
  const order = store.createOrder({ buyer_org_id: buyer.org_id, product: 'Cocoa', quantity: 1, price_usd: 100 });
  const pi = store.createPaymentIntent({ order_id: order.id, buyer_org_id: buyer.org_id, amount: 100, currency: 'TTD', method: 'wam', wam_payment_id: 'pay_abc', wam_checkout_url: 'https://billing.wam.money/pay/invoice/x' });
  const r = store.markPaymentByWamRef('pay_abc', 'PAID', 'completed');
  assert.equal(r.ok, true);
  assert.equal(r.payment.status, 'PAID');
  assert.ok(store.listPayments(buyer.org_id).length === 1);
});
