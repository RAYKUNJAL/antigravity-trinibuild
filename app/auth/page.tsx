...
function AuthPageInner() {
  ...
  const handleWhatsAppSignup = () => {
    const msg = encodeURIComponent('Hi! I want to sign up for TriniBuild. Please send me the signup link.');
    window.open(`https://wa.me/18683012345?text=${msg}`, '_blank');
      ...
      >
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl border-2 border-gray-100 mb-6">
          <div className="space-y-3 mb-6">
            ...
            <button onClick={handleWhatsAppSignup} disabled={loading}
          ...
          <form onSubmit={handleEmailAuth} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)}
                    placeholder="Ray Kunjal" required
            ...
            {error && <div className="bg-red-50 border-2 border-red-200 rounded-xl p-3 text-sm text-red-700 font-inter">{error}</div>}
            {message && (
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" /><span>Processing...</span></> : <span>{mode === 'signup' ? 'Create Account' : 'Sign In'}</span>}
            </button>
          </form>
        </div>

        <div className="text-center">
          <button onClick={() => router.push(`/auth?mode=${mode === 'signup' ? 'signin' : 'signup'}`)}
            className="text-gray-600 font-inter hover:text-trini-red transition-colors">
            {mode === 'signup' ? <>Already have an account? <span className="font-bold">Sign In</span></> : <>Do not have an account? <span className="font-bold">Sign Up</span></>}
          </button>
        </div>

        <div className="mt-8 text-center text-sm text-gray-500 font-inter">
          <p className="mb-2">Made for Trinidad and Tobago</p>
