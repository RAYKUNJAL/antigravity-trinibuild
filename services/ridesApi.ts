...
export async function fetchListedRides() {
  const res = await fetch('/api/rides/listed', { headers: { Accept: 'application/json' } });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data || data.listedCount === 0 || !Array.isArray(data.listed)) {
    return {
      unavailable: true,
      listedCount: 0,
      listed: [] as Array<{ id: string; name: string; plate: string; phone: string; wamHandle?: string }>,
