...
async function insertSampleData() {
    ...
    try {
        // 1. Add benefits column to jobs table (if not exists)
        console.log('📋 Step 1: Adding benefits column to jobs table...');
        const { error: alterError } = await supabase.rpc('exec_sql', {
            sql: 'ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS benefits TEXT[];'
        });
        if (alterError && !alterError.message.includes('already exists')) {
            console.warn('⚠️  Column may already exist:', alterError.message);
        } else {
            console.log('✅ Benefits column ready\n');
        }

        // 2. Insert classified listings
        console.log('📋 Step 2: Inserting classified listings...');
        const classifieds = [
        ...
        const { data: classifiedsData, error: classifiedsError } = await supabase
            ...
            .upsert(classifieds, { onConflict: 'id', ignoreDuplicates: true });

        if (classifiedsError) console.error('❌ Classifieds error:', classifiedsError);
        else console.log(`✅ Inserted ${classifieds.length} classified listings\n`);

        // 3. Insert jobs
        console.log('📋 Step 3: Inserting jobs...');
        const jobs = [
        ...
        else console.log(`✅ Inserted ${jobs.length} jobs\n`);

        // 4. Insert real estate listings
        console.log('📋 Step 4: Inserting real estate listings...');
        const realEstate = [
        ...
        const { data: realEstateData, error: realEstateError } = await supabase
            ...
            .upsert(realEstate, { onConflict: 'id', ignoreDuplicates: true });

        if (realEstateError) console.error('❌ Real estate error:', realEstateError);
        else console.log(`✅ Inserted ${realEstate.length} real estate listings\n`);

        // 5. Insert events
        console.log('📋 Step 5: Inserting events...');
        const events = [
        ...
        // 7. Insert success stories
        console.log('📋 Step 7: Inserting success stories...');
        ...
        const { data: storiesData, error: storiesError } = await supabase
            ...
            .upsert(successStories, { onConflict: 'id', ignoreDuplicates: true });

        if (storiesError) console.error('❌ Success stories error:', storiesError);
        else console.log(`✅ Inserted ${successStories.length} success stories\n`);

        // 8. Insert video placements
        console.log('📋 Step 8: Inserting video placements...');
        ...
        const { data: videoData, error: videoError } = await supabase
            ...
            .upsert(videoPlacements, { onConflict: 'id', ignoreDuplicates: true });

        if (videoError) console.error('❌ Video placements error:', videoError);
        else console.log(`✅ Inserted ${videoPlacements.length} video placements\n`);
